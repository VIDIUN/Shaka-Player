...
shaka.media.DrmEngine.prototype.loadOfflineSession_ = function(sessionId) {
  ...
  // return session.load(sessionId).catch(function(error) {
  //   console.log("error: " + JSON.stringify(error));
  //   return session.close().then((data)=>{
      return session.close()//.then(session.close())
         .then(session.load(sessionId).then(function(present) {
        return this.onSessionLoad(present, activeSession, session);
        // }));
      }.bind(this)).then(function(present) {
        return this.onSessionLoad(present, activeSession, session);
        ...
        return session.remove().then(session.load(sessionId).then(present=>{
          return this.onSessionLoad(present, activeSession, session);
        }));
      });
...
shaka.media.DrmEngine.prototype.onSessionLoad = function(present, activeSession, session){
  if (this.destroyed_) return;
