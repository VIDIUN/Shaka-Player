...
shaka.offline.DownloadManager = function(
  ...
  /** @public {?boolean} */
  this.storeManifestFirstMode = false;
};
 ...
 */
shaka.offline.DownloadManager.prototype.storeManifestFirst =
    function(manifest) {
  this.storeManifestFirstMode = true;
  return this.downloadAndStore(manifest).then((id)=>{
    this.storeManifestFirstMode = false;
...
shaka.offline.DownloadManager.prototype.downloadAndStore =
  ...
  let async = MapUtils.values(this.segments_).map(function(segments) {
    ...
    let downloadNext = (function() {
      ...
      return this.downloadSegment_(segment, manifestId, manifest)
          .then(downloadNext);
    }.bind(this));
    return downloadNext();
  }.bind(this));
  ...
  this.promise_ = Promise.all(async).then(function() {
    return manifestId ?
        this.storageEngine_.updateManifest(manifestId, manifest) :
...
shaka.offline.DownloadManager.prototype.downloadSegment_ =
  ...
  if (this.storeManifestFirstMode || this.manifest_.downloadStatus == 'paused'
  ...
  return this.netEngine_.request(type, request).promise
      ...
      .then(function(id) {
        ...
        if (manifestId!=null && currentManifest != null) {
          this.storageEngine_.updateManifest(manifestId, currentManifest);
        }
