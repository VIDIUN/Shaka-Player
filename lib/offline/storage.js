...
shaka.offline.Storage.prototype.downloadAndStoreManifest_ = function(
  ...
  return this.downloadManager_.storeManifestFirst(manifestDb).then((id)=>{
    this.downloadManager_.offlineManifestStorageId = id;
    ...
    this.downloadManager_.addSegment(
        ...
        function(id) {
          ...
          if (id == null) {
            ...
            for (let i = 0; i < streamDb.segments.length; i++) {
              if (streamDb.segments[i].startTime === startTime
                  && streamDb.segments[i].endTime === endTime) {
                streamDb.segments[i] = segmentDb;
                break;
              }
            }
          }
