...
shaka.offline.Storage.prototype.renewLicense = function(offlineUri) {
  ...
  return this.initIfNeeded_().then(function () {
    ...
    drmEngine.init(manifest, true).then(() => {

      let session = drmEngine.mediaKeys_.createSession('persistent-license');
      return session.load(manifestDB.sessionIds[0]).then(()=>{
        return session.remove().then(()=>{
          return this.loadInternal(
              manifestDB.originalManifestUri, onError).then((data) => {
            // Save these references so that they can be accessed elsewhere.
            this.manifest_ = data.manifest;
            this.drmEngine_ = data.drmEngine;

            /** @type {!Array<shakaExtern.Period>} */
            const storedPeriods = manifestDB.periods;

            const size = manifestDB.size;

            // Re-filter now that DrmEngine is initialized.
            this.filterAllPeriods_(data.manifest.periods);

            this.pendingContent_ = OfflineUtils.createStoredContentFromManifest(
                manifestDB.originalManifestUri,
                data.manifest,
                0,  // start with a size of 0
                manifestDB.appMetadata);

            /** @type {shakaExtern.ManifestDB} */
            let manifestDb = this.createOfflineManifest_(
                manifestDB.originalManifestUri, manifestDB.appMetadata);
            manifestDb.periods = storedPeriods;
            session = this.drmEngine_.activeSessions_[0].session.close();
            if (manifestId != null) {
              return this.storageEngine_.updateManifest(
                  manifestId, manifestDb);
            }
          }).catch(function (err) {
            // If we already had an error, ignore this error to avoid hiding
            // the original error.
